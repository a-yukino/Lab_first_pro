...
def about_page():
    ...
    st.image("Nanami.jpg", caption="画像のキャプション", use_column_width=True)
    st.write("対ロシア戦において、相手がオポジットでもミドルブロッカーでもブロックの高さが最も高い選手を起用するとすると、日本はアウトサイドヒッターで最も最高到達点が高い選手を起用し、さらにストレートを狙って攻撃すると良いと考えられる。")

    st.image("Yuka.jpg", caption="画像のキャプション", use_column_width=True)
    st.write("対ロシア戦において、相手がアウトサイドヒッターでもミドルブロッカーでもブロックの高さが最も高い選手を起用するとすると、日本はオポジットで最も最高到達点が高い選手を起用し、さらにストレートを狙って攻撃すると良いと考えられる。")
    st.write("この例を見てみると日本は身長が低い方なので、アウトサイドヒッターで攻撃することを提案する。")
